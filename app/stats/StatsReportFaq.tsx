...
const FAQ_ITEMS: FaqItem[] = [
  ...
  },
  {
    ...
    a: "클러스터링 스크립트 기준으로는 (1) 일봉 파일이 없거나 기사 공개일 이후 첫 종가를 못 찾은 경우, (2) 해당 기간(T+1/3/5)만큼 **미래 거래일 종가가 부족**한 경우가 제외에 포함됩니다. 제외 티커 목록(`excluded_tickers`)에 있는 종목은 애초에 분석에서 빠집니다.",
  },
  {
    q: "Baseline의 n=637과 상단 1038, 입장·보유 표의 n이 왜 다른가요?",
    a: "**서로 다른 JSON·스크립트**에서 나온 값입니다. Baseline(`advanced_stats.json`)은 `pharm_articles_manual_sentiment.json`과 장 마감 전후에 따른 T0 조정, T0 종가→T+N 종가 수익으로 집계된 **637건**입니다. 상단 1038은 `news_tickers.json` 기반 클러스터링의 (기사×종목×기간) 관측 합입니다. 입장·보유 표(`entry_hold_stats.json`)는 **`somedaynews_article_tickers.json`에서 유료 기사에 붙은 종목**을 한 건씩(같은 기사가 여러 레코드면 각각), 해당 종목이 **`data/eodhd_news_windows/manifest.json`** 에서 인트라데이+EOD를 갖는 경우만 대상으로 **EODHD 5분봉**만으로 진입·청산 가격을 잡은 A~E 그리드입니다. 표본 수는 입장 종류·해당 일 5분봉 유무에 따라 칸마다 약간씩 다를 수 있으며, Baseline·클러스터링과 **동일 유니버스가 아닙니다.**",
  },
  {
    q: "5분봉으로 정의한 입장·청산은 실제로 체결 가능한가요?",
    a: "백테스트는 **해당 5분봉의 시가/종가에 체결된다고 가정**한 근사입니다. 호가 스프레드·부분 체결·상한가·뉴스 확인 지연 등은 반영하지 않았습니다. 뉴스 직후 구간에서는 실제 체결가가 가정과 다를 수 있습니다.",
  },
  {
  ...
  {
    q: "입장·보유 ‘최적’은 긍정 신호만 골라서 본 건가요?",
    a: "`entry_hold_stats.json`은 `scripts/entry_hold_analysis.py`가 **`somedaynews_article_tickers.json` 기준 유료 기사에 붙은 종목**(티커당 한 번이 아니라 기사마다 붙은 종목 전부)에 대해 **A~E 입장×보유일 그리드 전체**를 계산한 뒤, 그중 **승률 최대 칸·평균 수익 최대 칸**을 요약한 것입니다. **긍정 신호로 먼저 걸러지지는 않습니다.** (과거 UI 문구의 ‘퀀트 긍정 신호’는 시그널과 함께 읽히게 하려는 표현이었습니다.)",
  },
  {
  ...
  },
  {
    ...
    a: "각 스크립트는 기사 공개일·당시 일봉만 사용해 T0(또는 그 다음 거래일)를 잡고, **그 시점 이후 종가·시가**로 수익을 냅니다. 다만 **뉴스 본문 NLP나 사후에 정의된 라벨**을 같은 시점에 쓸 수 있었는지는 데이터 소스별로 따로 점검이 필요합니다.",
  },
  {
