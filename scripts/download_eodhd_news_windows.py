...
(기본 before=60: MA20·거래량비 등 퀀트 지표에 최소 ~21거래일 히스토리 여유)
...
기존 캐시·매니페스트 연동:
- 이미 받은 eod/intraday JSON의 `from`~`to`가 **이번에 필요한 window보다 좁으면** 해당 구간만 API로 다시 받아 덮어씀 (`--force` 없이도).
- `manifest_per_article.json`은 기본적으로 **기존 행을 유지**하고, 이번 실행에 나온 (article_id×ticker)만 갱신 (--no-merge-manifest 로 전량 교체 가능).
- 웹앱은 **EOD+5분** 행만 사용합니다. 일봉만 남은 행·파일은 `scripts/prune_manifest_eod_only_rows.py`로 제거할 수 있습니다.
...
def manifest_row_key(r: dict) -> tuple[str, str]:
    """매니페스트 병합 키: (article_id, ticker)."""
    return (str(r.get("article_id", "")), str(r.get("ticker", "")))


def load_manifest_index(path: Path) -> dict[tuple[str, str], dict]:
    """기존 manifest_per_article.json → (article_id, ticker) → row. 손상 시 빈 dict."""
    if not path.is_file():
        return {}
    try:
...
def run_article_event_download(
    ...
    *,
    ...
    manifest_index: dict[tuple[str, str], dict] = (
        {} if args.no_merge_manifest else load_manifest_index(manifest_path)
    )
    if manifest_index:
        print(
            f"기존 매니페스트 {len(manifest_index)}행 로드 → 이번 실행에서 갱신되는 행만 덮어씀",
            flush=True,
        )

    def write_manifest_sorted() -> None:
        merged: dict[tuple[str, str], dict] = dict(manifest_index)
        for r in rows_by_idx:
            if r is not None and str(r.get("ticker", "")).strip():
                merged[manifest_row_key(r)] = r
        done = sorted(
            merged.values(),
            key=lambda r: (int(r.get("article_idx", 0)), str(r.get("ticker", ""))),
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=60,
        help="t0 이전 달력일(거래일 아님). 퀀트 MA20/거래량비 등에 ~21거래일 필요 → 기본 60",
    )
    ...
    parser.add_argument(
        "--no-merge-manifest",
        action="store_true",
        help="매니페스트를 기존 파일과 합치지 않고 이번 실행 행만으로 덮어씀 (부분 실행 시 주의)",
    )
    parser.add_argument(
        "--token-file",
        default="",
